...
export class PgnReader {
  ...
  loadPgn(): PgnReader {
    ...
    let _mgame = parse(this.configuration.pgn, {
      startRule: "game",
    }) as unknown as ParseTree;
    this.games = [_mgame];
    this.loadOne(_mgame);
    return this;
  }
  ...
  loadMany() {
    this.games = parse(this.configuration.pgn, {
      startRule: "games",
    }) as unknown as ParseTree[];
  }
  ...
  getGame(index: number): PgnGame {
    ...
    return {
      moves: this.getMoves(),
      gameComment: this.getGameComment(),
      tags: tags,
    };
  }
  ...
  isDeleted(id: number): boolean {
    if (!this.isMove(id)) return true; // Every non-existing moves is "deleted"
