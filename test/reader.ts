...
reader("should understand comments for variation with white", function () {
  const reader = new PgnReader({
    pgn: "1. d4 ({START} 1. e4 {AFTER} e5) 1... d5",
  });
...
reader("should understand both circles and arrows", function () {
  const reader = new PgnReader({
    pgn: "e4 {[%csl Yf4,Gg5,Gd4,Rc4,Bb4,Ya4][%cal Gg1f3,Rf1c4,Gh2h4,Rg2g4,Bf2f4,Ye2e4]}",
  });
  const first = reader.getMove(0);
...
tags("should read unusual spacing of tags", () => {
  let pgn = new PgnReader({
    pgn: '[  White    "Me"   ]  [  Black  "Magnus"   ] 1. e4',
  });
  let tags = pgn.getTags();
...
variations("should understand nested variations", () => {
  const reader = new PgnReader({
    pgn: "1. e4 e5 (d5 2. exd5 Qxd5 (2... Nf6))",
  });
  assert.equal(reader.getMove(1).variations[0].notation.notation, "d5");
...
iterating("should find all variations", () => {
  const moves = new PgnReader({
    pgn: "1. e4 e5 (1... d5) 2. d4 (2. Nf3 Nc6)",
  }).getMoves();
...
iterating("should find nested variations", () => {
  const moves = new PgnReader({
    pgn: "1. e4 e5 (1... d5) 2. Nf3 Nc6 (2... d6 3. d4 (3. Be2)) 3. Bb5",
  }).getMoves();
...
iterating("should find follow-ups of nested variations", () => {
  const moves = new PgnReader({
    pgn: "1. e4 e5 2. Nf3 (2. f4 exf4 (2... d5) 3. Nf3 {is hot}) 2... Nc6",
  }).getMoves();
...
defaultRead("should read one variation for white with move after", () => {
  const reader = new PgnReader({
    pgn: "1. e4 e5 2. f4 (2. Nf3 Nc6) 2... exf4 3. Nf3",
  });
  const moves = reader.getMoves();
...
sanSuite("should emmit correct discriminator", () => {
  let reader = new PgnReader({
    position: "rnbqkbnr/p1p2p2/3p2p1/R3p2p/7P/8/1PPPPPP1/RNBQKBN1 w kq - 0 8",
  });
...
deletingLines("should delete the whole variation with the first move", () => {
  const reader = new PgnReader({
    pgn: "1. e4 e5  2. Nf3 (2. f4 exf4) (2. d4 exd4) Nc6",
  });
  assert.equal(reader.getMove(3).variationLevel, 1);
